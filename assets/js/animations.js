...
jQuery(document).ready(function($) {
    ...
    if (isMobile.any === false) { 
        ...
        $('#promo .heading').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
                if (isInView) {$(this).addClass('animated fadeInDown delayp1');}
            });
        });
        
        $('#promo .intro').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
        ...
        $('#services .custom-icon').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
                if (isInView) {$(this).addClass('animated fadeInUp delayp1');}
            });
        });
        ...
        $('#why .badge-item').css('opacity', 0).each(function(i, el){
            $(el).one('inview', function(isInView) {
